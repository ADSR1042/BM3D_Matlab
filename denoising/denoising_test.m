%用于计算psnr的演示脚本
function [res0,res1] = denoising_o2(noisy_image,sigma)
    block_length = 8;
    step = 4;
    ...
    for i = (1:block_num)
        ...
        for j=(1:length(scope))
            Z_x =  block(:,:,scope(j));
            temp = wthresh(DCT_2D(Z_xr),'h',upsilon_ht) - wthresh(DCT_2D(Z_x),'h',upsilon_ht);
            block_distant_xr(j) = norm(temp)^2/(block_length^2); %TODO 暂不清楚 N1_ht作用 暂时用block_length代替
       end
       %处理block_distant_xr
       %升序排序 优先获取距离小的 并记录序列
    ...
    for i = 1:block_num
        %顺带计算N
        temp = wthresh(DCT_3D(Z_sht_xr{i}),'h',upsilon_3D);
